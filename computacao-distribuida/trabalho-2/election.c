...
void send_to_id(int to, int id, char message[ML])
{
    ...
    sendto(id, (const char *)message, strlen(message),M_CONFIRM,(const struct sockaddr *)&cl,sizeof(cl));
}
...
int main(int argc, char* argv[])
{
    ...
    while(TRUE)
    {
        ...
        {
            strcpy(message, "E-ACK"); // send election

            sendto(sock_id, (const char*) message, strlen(message), M_CONFIRM, (const struct sockaddr *)&from, sizeof(from));
